...
RAFT_config = {
    "pretrained": "./pretrained_models/raft-things.pth",
    "iters": 12,
    "dropout": 0.0,
    "small": False,
    "pose_scale": 1.0,
...
def reformat_dataset(data_dir, start_frame, end_frame, img_size=(512, 640), dst_dir_base = None, skip_depth_infer = False):
    ...
    gen_right = False
    if gen_right:
        image_dir_right = os.path.join(output_dir, 'images_right')
        if not os.path.exists(image_dir_right):
            os.makedirs(image_dir_right)

    mask_dir = os.path.join(output_dir, 'masks_ori')
    depth_dir = os.path.join(output_dir, 'depth')
    ...
    for i, frame in enumerate(tqdm(frames)):
        ...
        if skip_depth_infer:
            ...
            with torch.no_grad():
                depth = depth_estimator(left_img[None], right_img[None], baseline[None])
        try:
            mask = read_mask(os.path.join(data_dir, 'masks', frame))# read and save ori SM mask in masks_ori
        ...
        name = 'frame-'+str(i).zfill(6)+'.color.png'
        if gen_right:
            right_img_np = right_img.permute(1, 2, 0).numpy()
            right_img_bgr = cv2.cvtColor(right_img_np, cv2.COLOR_RGB2BGR)
            cv2.imwrite(os.path.join(image_dir_right, name), right_img_bgr)
