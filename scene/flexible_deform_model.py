...
class TissueGaussianModel:
    ...
    def load_model(self, path):
        ...
        if os.path.exists(os.path.join(path, "deformation_table.pth")):
            ...
            assert 0, f"resume?deformation_table.pth{os.path.join(path, 'deformation_table.pth')}"
            # assert 'endog' in path, f"resume for other than endog method?deformation_table.pth{os.path.join(path, 'deformation_table.pth')}"
        self._deformation_accum = torch.zeros((self.get_xyz.shape[0],3),device="cuda")
